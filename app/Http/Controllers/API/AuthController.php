...
class AuthController extends BaseController
{
    ...
    public function loginByFaceId(LoginFaceIdRequest $request, AuthAction $action)
    {
        $user = User::where('face_id', $request->face_id)->first();
        if (!$user) {
            return $this->sendError('G00050005');
        }
        return $action->login($user);
    }
